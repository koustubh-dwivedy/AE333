function [I_mat_new] = I_matrix_new(num_panels, x_vals_start, y_vals_start, x_vals_centre, y_vals_centre, x_vals_end, y_vals_end)

I_mat_new = zeros(num_panels, num_panels);
...
for i = 1:num_panels
	for j = 1:num_panels
		if i ~= j
			...
			I_mat_new(i, j) = ((D_val - A_val*C_val)/(2*E_val))*log((S_j_val*S_j_val + 2*A_val*S_j_val + B_val)/B_val) - C_val*(atan((S_j_val + A_val)/E_val) - atan(A_val/E_val));
		end
	end
end
